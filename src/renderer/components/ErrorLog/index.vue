<template lang="pug">
  div(v-if="errorLogs.length>0")
    el-badge(:is-dot="true" style="line-height: 30px;" @click.native="dialogTableVisible=true")
      el-button.bug-btn(size="small" type="danger")
      ...
      el-table(:data="errorLogs" border)
        el-table-column(label="Message")
          template(slot-scope="scope")
            div
              span.message-title Msg:
              el-tag(type="danger") {{ scope.row.err.message }}
            br
            div
              span.message-title(style="padding-right: 10px;") Info:
              el-tag(type="warning") {{scope.row.vm.$vnode.tag}} error in {{scope.row.info}}
            br
            div
              span.message-title(style="padding-right: 16px;") Url:
              el-tag(type="success") {{scope.row.url}}
        el-table-column(label="Stack")
          template(slot-scope="scope") {{scope.row.err.stack}}
</template>
