  ...
  .login-container
    el-form.login-form(autoComplete="on" :model="loginForm" :rules="loginRules" ref="loginForm" label-position="left")
      ...
      el-form-item(prop="pasword")
        ...
        el-input(name="pasword" :type="passwordType" @keyup.enter.native="handleLogin" 
                 v-model="loginForm.password" autoComplete="on" :placeholder="$t('login.password')")
        span.show-pwd(@click="showPwd")
          svg-icon(:icon-class="passwordType===''?'eye.1':'eye'")
      
      el-button(type="primary" style="width:100%;margin-bottom:30px;" :loading="loading" @click.native.prevent="handleLogin") {{$t('login.logIn')}}
